...
public class PushComponentAction extends ContextAwareComponentAction {
  ...
  public void actionPerformed(AnActionEvent anActionEvent, TreePath path, Object selected, Odo odo) {
    ...
    CompletableFuture.runAsync(() -> {
      try {
        if (checkMigrated(odo, projectNode.toString(), applicationNode.toString(), component)) {
  ...
  private boolean checkMigrated(Odo odo, String project, String application, Component component) throws IOException {
    if (component.getState() == ComponentState.PUSHED) {
      ComponentInfo info = odo.getComponentInfo(project, application, component.getName(), component.getComponentKind());
      if (info.isMigrated()) {
        ...
        if (choice == 0) {
          odo.undeployComponent(project, application, component.getPath(), component.getName(), component.getComponentKind());
        } else if (choice == 1) {
