...
public class PushComponentAction extends ContextAwareComponentAction {
  ...
  private boolean checkMigrated(Odo odo, String project, String application, Component component) throws IOException {
    ...
    if (component.getState() == ComponentState.PUSHED) {
      ComponentInfo info = odo.getComponentInfo(project, application, component.getName());
      if (info.isMigrated()) {
        ...
        if (choice == 0) {
          odo.undeployComponent(project, application, component.getPath(), component.getName(), component.getInfo().getComponentType().getKind());
        } else if (choice == 1) {
