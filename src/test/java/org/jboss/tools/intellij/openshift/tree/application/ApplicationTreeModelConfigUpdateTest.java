...
public class ApplicationTreeModelConfigUpdateTest extends BasePlatformTestCase {
    ...
    public void testShouldRefreshIfContextHasNoChanges_butConfigHasErrors() {
        // given
        var error = mock(Exception.class);
        Config clone = createConfig(config);
        // when
        model.onUpdate(clone, error);
        // then
        verify(model).refresh();
    }

    public void testShouldNotRefreshIfContextHasNoChanges_andConfigHasNoErrors() {
        // given
        Config clone = createConfig(config);
        // when
        model.onUpdate(clone, null);
        // then
        verify(model, never()).refresh();
    }

    public void testShouldRefreshIfCurrentContextHasDifferentName_andConfigHasNoErrors() {
        // given
        ...
        // when
        model.onUpdate(differentUser, null);
        // then
        verify(model).refresh();
    }

    public void testShouldRefreshIfCurrentContextHasDifferentCluster_andConfigHasNoErrors() {
        // given
        ...
        when(differentCluster.getCurrentContext())
          .thenReturn(currentContext);
        // when
        model.onUpdate(differentCluster, null);
        // then
        verify(model).refresh();
    }

    public void testShouldRefreshIfCurrentContextHasDifferentNamespace_andConfigHasNoErrors() {
        // given
        Config differentCluster = createConfig(config);
        NamedContext currentContext = createNamedContext(differentCluster.getCurrentContext());
    ...
    public void testShouldRefreshIfCurrentContextHasDifferentUser_andConfigHasNoErrors() {
        // given
        Config differentUser = createConfig(config);
        NamedContext currentContext = createNamedContext(differentUser.getCurrentContext());
    ...
    public void testShouldRefreshIfCurrentContextHasDifferentToken_andConfigHasNoErrors() {
        // given
        ...
        when(differentToken.getCurrentContext())
          .thenReturn(currentContext);
        // when
        model.onUpdate(differentToken, null);
        // then
        verify(model).refresh();
    }
