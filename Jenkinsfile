	...
	try {
		...
		if(params.UPLOAD_LOCATION) {
			stage('Upload') {
				...
				sh "sftp -C ${UPLOAD_LOCATION}/snapshots/intellij-openshift-connector/ <<< \$'put -p ${filesToPush[0].path}'"
		...
		if(publishToMarketPlace.equals('true')){
			timeout(time:5, unit:'DAYS') {
				input message:'Approve deployment?', submitter: 'sbouchet'
			}
			...
			stage("Publish to Marketplace") {
				...
				if (!isSnapshot) {
					stage("Promote the build to stable") {
						...
						sh "sftp -C ${UPLOAD_LOCATION}/stable/intellij-openshift-connector/ <<< \$'put -p ${zip[0].path}'"
